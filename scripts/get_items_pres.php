...
try{
	...
	$sqlArt="SELECT
		presupuesto_articulos.id_item,
		presupuesto_articulos.proveedor,
	...
	INNER JOIN articulos ON presupuesto_articulos.id_articulo=articulos.id_articulo
	INNER JOIN listado_precios ON presupuesto_articulos.id_articulo=listado_precios.id_articulo
	WHERE articulos.id_empresa=$id_empresa AND folio=$idCot;";	
	$res=$bd->query($sqlArt);
	...
	foreach($res->fetchAll(PDO::FETCH_ASSOC) as $v){
		...
		<tr id="'.$id.'" class="lista_articulos verde_ok">
			...
			<td> <input class="proveedor" type="text" value="'.$v["proveedor"].'" size="10" onkeyup="prov_completar()"></td>
	...
	$sqlPaq="SELECT
		...
		cantidad*precio as total
	FROM presupuesto_articulos
	INNER JOIN paquetes ON presupuesto_articulos.id_paquete=paquetes.id_paquete
	INNER JOIN listado_precios ON presupuesto_articulos.id_paquete=listado_precios.id_paquete
	WHERE paquetes.id_empresa=$id_empresa AND folio=$idCot ;";	
	$res=$bd->query($sqlPaq);
